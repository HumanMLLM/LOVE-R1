...
- In the reasoning, you should specify how you localize the specific video segment based on tracking the development and transformation of events in the video.
- The segment should be presented as [start_time, end_time] in integer seconds.
- The captions reflect what is present in the video. Do not mention 'caption', 'description', 'describe', or other similar words in your reasoning
...
"""
    ...
    # try:
    #     prompt = example['caption'] + SYSTEM_PROMPT1 + SYSTEM_PROMPT3 + ' Hint: the relavent video segment is [%d, %d]. But do not mention the hint in your reasoning.' % (int(example["clue_intervals"][0][0]), int(example["clue_intervals"][0][1]))
    ...
    #     # print(message)
    #     print(SYSTEM_PROMPT1 + SYSTEM_PROMPT3 + ' Hint: the relavent video segment is [%d, %d]. But do not mention the hint in your reasoning.' % (int(example["clue_intervals"][0][0]), int(example["clue_intervals"][0][1])))
    #     # info = gpt_api(message, "qwen-vl-max-latest")
    #     info = gpt_api(message, "gemini-2.5-pro")
    #     # info = gpt_api(message, "qwen3-235b-a22b-instruct-2507")
    #     # info = gpt_api(message, "gpt-4o-2024-11-20")
    ...
    #     #     answer = info.split("\\boxed{")[1].split("}")[0]
    #     #     info = infos + "\\boxed{" + answer + '}'
    #     #     if iou_1d(eval(answer), example['clue_intervals'[0]]) > 0.7:
    #     sample_cot = info
    # #         # return info
...
# 配置
INPUT_JSON = "subset1.json"          # 输入的原始数据文件（包含一个列表）
OUTPUT_JSONL = "cgbench_output1_answer_cot.jsonl"      # 输出文件（JSON Lines 格式）
